...
@include('adminlte::layouts.partials.header')
	<div class="container">
		<div class="wrapper">
			<div class="box-sect-content result">
				<div class="box-left">
							...
							<div class="col-lg-6">
								<select class="form-control custom-1" id="dropdownlokasi" name="lokasi">
									...
									@foreach($tampilkawasan as $tampilkawasan)
										<option value="{{$tampilkawasan->id_kawasan}}">{{$tampilkawasan->nama_kawasan}}</option>
								    @endforeach
							    </select>
						    </div>
			       			<div class="col-lg-12">
			    ...
			    @endforeach
				<div class="box-right">
					<h3>Menampilkan pencarian untuk "{{$jenis}} <?php echo $sta1; ?> di <?php echo $_GET['alamat']; ?>"</h3>
					<span class="count">(<?php echo $count; ?> hasil pencarian)</span>
					<ul class="listing-view">
						<li class="list-view">Urutkan Berdasarkan: </li>
						<li class="list-view"><a href="{{url($aa)}}&view=" class="<?php if($_GET['view']=='') { echo 'active';}?>">Rekomendasi</a></li>
						<li class="list-view"><a href="{{url($aa)}}&view=terbaru" class="<?php if($_GET['view']=='terbaru') { echo 'active';}?>">Terbaru</a></li>
						<li class="list-view"><a href="{{url($aa)}}&view=populer" class="<?php if($_GET['view']=='populer') { echo 'active';}?>">Terpopuler</a></li>
						<li class="list-view tree"><a href="#"  class="<?php if($_GET['view']=='hargarendah'||$_GET['view']=='hargatinggi') { echo 'active';}?>">Harga <i class="fa fa-angle-left pull-right"></i></a>
							<ul class="sub-listing-view">
								<li class="sub-list-view"><a href="{{url($aa)}}&view=hargatinggi">Tinggi ke Rendah</a></li>
								<li class="sub-list-view"><a href="{{url($aa)}}&view=hargarendah">Rendah ke Tinggi</a></li>
							</ul>
						</li>
					</ul>
							...
							<li class="list-property">
								<?php 
									...
									if ($tampilpropertys->property_status=="dijual;disewa") {
										...
										if ($_GET['status']=='dijual') {
											# code...
											$stat = "dijual";	
										} elseif ($_GET['status']=='disewa') {
											# code...
											$stat = "disewa";
											$text = "<span>/ thn</span>";
								...
								<?php if ($tampilpropertys->property_image!='') {
									...
									?>
								<div class="box-image">
									<a href="{{ url('detail',$tampilpropertys->property_id) }}"><img src="{{ asset('/img/property') }}/{{$img}}" class="image-property"></a>
									<?php echo $textharga;?>
								...
								<div class="content-property">
									...
									<div class="detail">
										...
										<p>Harga / Meter: <span>Rp. {{number_format($tampilpropertys->property_harga, 0)}}</span></p>
									</div>
									<?php
									} else {
										# code...
					...
					<div class="pagination">
						...
						{{ $tampilproperty->appends(Request::except('page'))->links() }}
					</div>
				</div>
			</div>
